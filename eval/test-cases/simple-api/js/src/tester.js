...
async function getAllStudents() {
  try {
    ...
    const expectedStudents = [new Student(1, 'Doe', 'John'), new Student(2, 'Doe', 'Jane')]
...
async function getExistingStudent() {
  try {
    ...
    if (student.id === 1 && student.firstName === 'Rainer' && student.lastName === 'Zufall') {
      successes.push('getExistingStudent')
    } else {
      console.log(`getExistingStudent failed: ${JSON.stringify(student)} did not match expected values`)
...
async function getNonExistingStudent() {
  try {
    ...
    if (e instanceof ApiException) {
      ...
      fails.push(
        `getNonExistingStudent:Expected exception to be instance of ApiException but got ${e.constructor.name}`
      )
    }
  }
}
...
async function getExamsOfExistingStudent() {
  try {
    ...
    const expectedExams = [new Exam(1, 'English'), new Exam(2, 'Math')]
    if (JSON.stringify(exams) === JSON.stringify(expectedExams)) {
      successes.push('getExamsOfExistingStudent')
    } else {
      console.log(
        `getExamsOfExistingStudent failed: ${JSON.stringify(exams)} did not match expected ${JSON.stringify(
          expectedExams
        )}`
      )
...
async function getExamsOfNonExistingStudent() {
  try {
    ...
    if (e instanceof ApiException) {
      ...
      fails.push(
        `getExamsOfNonExistingStudent:Expected exception to be instance of ApiException but got ${e.constructor.name}`
      )
    }
  }
}
